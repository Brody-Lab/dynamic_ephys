...
function [x, frbins, Pjoints, fr_given_as, fr_var_given_as, a_given_frs] = dyn_compile_binned_database(cellid, t0s, n_dv_bins, varargin)
...
if force || isempty(u) || ~exist([datadir filesep 'compiled_binned_' num2str(u) '.mat'], 'file'),

	[x, frbins, Pjoints_unbinned, ~, ~, a_given_frs] = dyn_compile_database(cellid, t0s, 'lag', lag, ...
		'frbins', frbins, 'dt', dt, 'trialnums', trialnums, 'use_nans', use_nans, 'alignment', alignment,...
        'krn_width', krn_width, 'fr_dt', fr_dt, 'force', 0, 'direction', direction, ...
    ...
    %% ALEX CODE - I'm pretty sure this is the first point where the sticky bounds come into play
...
keyboard 
    ...
    for time_i=1:numel(t0s)
        ...
        % bound bins
%%% DEBUG CODE
        Pjoints(time_i,:,1) = Pjoints_unbinned(time_i,:,1); 
        Pjoints(time_i,:,end) = Pjoints_unbinned(time_i,:,end); 
        % non-bound bins
        for j=2:n_dv_bins-1
