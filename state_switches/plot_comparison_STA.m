...
text(-.4, .99*max(ylim), 'triggered on','fontsize',dp.fsz)
text(-.4, .99*max(ylim)-.1*diff(ylim), 'generative switches','color',gen_color,'fontsize',dp.fsz)
text(-.4, .99*max(ylim)-.2*diff(ylim), 'model switches','color',dp.model_color,'fontsize',dp.fsz)
ax = gca;
xlim(ax,[-.65 .65])
plot([0 0], [-1 100], 'k--')
...
legend boxoff
if savefig
print(fh1, fullfile(dp.sta_fig_dir,['STA_comparison_' which_correction_str ]),fig_type,'-painters')
end
